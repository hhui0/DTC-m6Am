...
class LitModel(object):
    ...
    def refresh(self):
        ...
        ckpt = ModelCheckpoint(self.__ckptdir, self.__ckptname+"-{epoch}", self.__monitor, mode=self.__mode,verbose=True)
    ...
    def fit(self, train_data: Dataset, valid_data: Optional[Dataset] = None, verbose=True):
        #随机采样
        ...
        train_iter = DataLoader(
            self.train_set, self.__batchsize if self.__batchsize > 0 else len(self.train_set), shuffle=config['shuffle'], generator=generator # type: ignore
        )
        if self.valid_set:
